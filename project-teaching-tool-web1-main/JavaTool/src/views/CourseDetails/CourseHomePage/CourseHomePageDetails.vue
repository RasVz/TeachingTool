<template>
  <a-card style="height: 100%;overflow-y: auto;">
    <a-tabs v-model:selectedTab="noticeTab">
      <a-tab-pane v-for="tab in noticeList" :key="tab.key" :tab="tab.tab">
        <a-table v-if="tab.key === 'tab1'" item-layout="horizontal" :columns="columns" :data-source="data"
          :pagination=false :scroll="{ y: '35vh' }">
        ...
        <a-table v-if="tab.key === 'tab2'" item-layout="horizontal" :columns="columns" :data-source="data"
          :pagination=false :scroll="{ y: '35vh' }">
          <template #bodyCell="{ column, text, record }">
            <template v-if="column.dataIndex === 'task_name'">
              <a @click="ViewTaskDetali(record)">{{ text }}</a>
