<template>
    <div>
        <a-modal v-model:open="visible.value" title="修改发布任务" ok-text="确认" cancel-text="取消" @ok="onOk" @cancel="formRef.resetFields()
        emit('update:modelVisible', false);" :maskClosable="false">
            <a-form ref="formRef" :model="formState" name="form_in_modal" :rules="rules" :label-col="labelCol">
                <a-form-item name="task_name" label="任务名称">
                    <a-input v-model:value="formState.task_name"  />
                </a-form-item>
        
                <a-form-item name="task_description" label="任务描述">
                    <a-textarea v-model:value="formState.task_description" />
                </a-form-item>

            
            </a-form>
        </a-modal>
    </div>
</template>
<script lang="ts" setup>
import { reactive, ref, toRaw, computed } from 'vue';
import type { FormInstance } from 'ant-design-vue';
import type { Rule } from 'ant-design-vue/es/form';
import type { UnwrapRef } from 'vue';






const labelCol = { style: { width: '90px' } };

//modelVisible用于开关窗口
const props = defineProps({
    modelVisible: {
        type: Boolean,
        default: false
    }
})
const emit = defineEmits(['update:modelVisible']);

const formRef = ref<FormInstance>();
interface Deliverable {
    deliverable_name: string;
    deliverable_doc: any[];
}
interface FormState {
    task_name: string;
    project_id: number;
    module_id: number,
    task_description: string;
    task_type: string;
    deliverables: Deliverable[]
}
const visible = computed(() => {
    return ref(props.modelVisible)
});


const formState: UnwrapRef<FormState> = reactive({
    task_name: '',
    project_id: null,
    module_id: null,
    task_description: '',
    task_type: 'A',
    deliverables: []
});

const onOk = () => {
    formRef.value
        .validateFields()
        .then(values => {
            console.log('Received values of form: ', values);
            console.log('formState: ', toRaw(formState));
            emit('update:modelVisible', false);
            formRef.value.resetFields();
            console.log('reset formState: ', toRaw(formState));
        })
        .catch(info => {
            console.log('Validate Failed:', info);
        });
};
//表单验证规则
const rules: Record<string, Rule[]> = {
    task_name: [
        { required: true, message: '请输入任务名称', trigger: 'change' },

    ],
    project_id: [
        { required: true, message: '请选择项目', trigger: 'change' },
    ],
    module_id: [
        { required: true, message: '请选择模块', trigger: 'change' },
    ],
    task_type: [{ required: true, message: '请选择任务类型', trigger: 'change' }],

    deliverable_name: [
        { required: true, message: '请输入交付物名称', trigger: 'blur' },
    ]
};


</script>
<style scoped></style>