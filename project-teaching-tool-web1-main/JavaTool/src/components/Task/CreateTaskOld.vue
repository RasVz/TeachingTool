<template>
  <div>
    <a-button type="primary" @click="visible = true" style="margin-right:5px">新增任务</a-button>
    <a-modal
    ...
    >
      <a-layout style="height: 500px; overflow: hidden">
        ...
        >
          ...
          <a-collapse
            v-if="!collapsed"
            v-model:activeKey="activeTaskKey"
            ...
            >
              ...
              <a-form-item name="task_project" label="所属项目">
                <a-select
                  ...
                  v-model:value="formState.task_project"
                  style="width: 90%"
                />
              </a-form-item>
              <a-form-item name="task_module" label="所属模块">
                <a-select
                  ...
                  v-model:value="formState.task_module"
                  style="width: 90%"
                />
              </a-form-item>
              <a-form-item name="task_team" label="承担小组">
                <a-select
                  ...
                  placeholder="选择小组"
                  :options="teamOptions"
                  v-model:value="formState.task_team"
                  style="width: 90%"
                  multiple
                />
              </a-form-item>
              <a-form-item name="task_endTime" label="任务期限">
                <a-date-picker
                  show-time
                  v-model:value="formState.task_endTime"
                  :locale="locale"
                  style="width: 90%"
                />
              </a-form-item>
              ...
              <div
                ...
                v-for="(item, index) in formState.deliverables"
                :key="index"
              ...
              >
                ...
                <a-form-item
                  name="deliverable_doc"
                  style="margin-left: 10px; width: 100%"
                  label="交付物模板"
                >
                  <a-upload
                    v-model:file-list="item.deliverable_doc"
                    name="file"
                    action=""
                    @change="handleChange"
                  >
...
const projectOptions = ref<SelectProps['options']>([
  ...
  },
  {
    value: 'project2',
    label: '项目2',
    children: [{ value: 'module3', label: '模块3' }]
  }
]);
...
const onProjectChange = (projectValue: string) => {
  const selectedProject = projectOptions.value.find((project) => project.value === projectValue);
  moduleOptions.value = selectedProject ? selectedProject.children : [];
...
.task-details {
  padding: 7px;
  cursor: pointer; /* 显示指针光标，表示可以点击 */
  transition: background-color 0.3s, border-color 0.3s;
}

.task-details:hover {
  background-color: #e6f7ff; /* 鼠标悬停时背景色变化 */
}

.task-details.active {
  background-color: #eef1f3; /* 点击后的背景色 */
}
